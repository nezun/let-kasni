...
- P1 visuals rule: every new or refreshed main/cornerstone guide needs 3-5 useful in-body visual or interactive elements where the format supports it. Every standard article needs 2-4. Eligible elements include calculator, table, timeline, checklist module, comparison card, route map, process visual, quick-check CTA module, or a relevant generated/selected image. Hero images, sticky/sidebar forms, and "Detaljni vodiči" / "Detailed guides" navigation cards do not count toward this minimum. Standard blog articles must use a wide reading layout, must not render the old sticky sidebar form, must include the approved in-body quick-check CTA after the first 2-3 H2 sections, and must include one strong context image from the article image pipeline. Visuals should come from competitor-observed patterns where possible and must be distributed through the article, roughly every 3-4 H2s depending on length, instead of clustered at the beginning or stacked one after another.
