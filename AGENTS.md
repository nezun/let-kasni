...
- Blog and guide generators must not create public H2s by prefixing the full article title plus a reusable suffix. Ban heading templates such as `{article title}: route, timing...`, `{article title}: documents...`, and public copy such as `detaljan deo šire teme`, `detailed part of the wider`, `arhiva vodiča`, or `should not be assessed in isolation`. Reusable sections are allowed only when the heading is a natural reader task and the body starts with the concrete passenger-rights fact, not with content hierarchy.
